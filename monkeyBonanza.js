class MonkeyBonanza {
 ...
 drawZigzagLine(x, y, length, separation, amplitude) {
  ...
  for (let i = 0; i < numSegments; i++) {
    line(x - length / 2 + i * separation, y, x - length / 2 + i * separation + separation / 2, y - amplitude);
    line(x - length / 2 + i * separation + separation / 2, y - amplitude, x - length / 2 + (i + 1) * separation, y);
  }
}
